...
| D-054 | El corte de interés es global, el día 1 de cada mes. El job revisa una vez al día, y al arrancar, si falta el cargo de algún período ya vencido y lo genera, por si el día 1 falló (N-01) | Decisión del usuario. Cuándo le toca el primer cargo a un préstamo nuevo sigue en N-04 |
| D-055 | En los reportes filtrados por tipo no se excluye nada: cada reversión cuenta junto con el tipo del asiento que reversa, con su signo, así un pago y su reversión suman 0 (P-12) | Decisión del usuario. Sin esto, la reversión de un `PAYMENT` quedaría fuera del filtro por ser de tipo `REVERSAL` |
...
| D-057 | `WRITTENOFF` (incobrable): el cliente ya no va a pagar (mala paga, fallecimiento, etc.). **Provisional:** deja de generar interés, si llega a pagar el pago se acepta, y solo lo marca un ADMIN (P-07) | Significado dado por el usuario; los tres detalles son la recomendación del agente y falta confirmarlos |
| D-058 | **Provisional:** un préstamo pasa solo a `CLOSED` cuando un pago o una condonación deja capital e interés en 0, y vuelve a `ACTIVE` si una reversión le devuelve saldo (P-06) | Recomendación del agente; el usuario no respondió P-06 explícitamente |
