...
| D-063 | Todo endpoint exige usuario autenticado (política por defecto), salvo `api/auth`. Solo ADMIN, por ahora: marcar incobrable (D-057) y el CRUD de usuarios. Lo demás lo puede hacer también un WORKER (P-18) | Recomendación del agente con lo que ya estaba decidido; qué otras acciones son solo del dueño queda en P-18 |
