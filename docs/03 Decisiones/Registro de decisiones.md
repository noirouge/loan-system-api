...
| D-076 | `GET api/users/options` lo usa cualquier usuario autenticado: lista solo usuarios activos con id, nombre, apellido y usuario, para elegir `counterpartyUserId`. El resto de `api/users` sigue siendo solo ADMIN | Pedido del usuario |
