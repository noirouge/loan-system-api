...
| POST | `api/loans/{id}/write-off` | Marca un préstamo activo como incobrable (D-057). Sigue aceptando pagos. Responde `204` | Implementado (#47, #67, #114) |
...
| GET | `api/reports/monthly-collection?month=yyyy-MM&paymentDay=` | Hoja de cobro del mes: por préstamo, cliente y código, número de pago, original, tasa, lo que debía al corte, interés y capital del mes, cuota, abono (interés y capital), lo que resta, estado y si estaba congelado; con totales (D-075) | Implementado (#117) |
